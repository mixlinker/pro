...
import lineModal from './component/line-modal.vue'
...
const getItem = () => {
    ...
    return new Promise((resolve, reject) => {
        loading.value = true
        proxy.api
            .post('retainer_result_with_events', data)
            .then((res) => {
                if (res.code === 200) {
                    loading.value = true
...
const getRecordList = () => {
    ...
    proxy.api
        .post('retainer_record_list', data)
        .then((res) => {
            if (res.code === 200) {
...
const getListHeight = () => {
    nextTick(() => {
        ...
        listHeight.value = height - infoHeight - 70 //总高度-基本信息高度54-列表区域上边16下边12
    })
}
